...
// MemVFS 是内存 VFS 实现：测试与一致性向量运行器使用（JS 端对齐同一行为）。
type MemVFS struct {
	root *memNode
}
...
func (m *MemVFS) Create(name string) (io.WriteCloser, error) {
...
func (m *MemVFS) WriteFile(name string, data []byte) error {
	w, err := m.Create(name)
	if err != nil {
		return err
	}
...
func (m *MemVFS) MkdirAll(name string) error {
	name = path.Clean(name)
	if name == "/" {
		return nil
	}
