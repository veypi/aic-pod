...
func (b *Bytes) CloseOwner(owner string) {
	b.mu.Lock()
	if b.pendingOwners[owner] > 0 {
...
func (b *Bytes) Reap() {
	...
	for _, s := range b.sources {
		if time.Now().After(s.expires) {
			expired = append(expired, key{s.owner, s.descriptor.Ref})
		}
	}
	b.mu.Unlock()
	for _, s := range expired {
		_ = b.Release(s.owner, s.ref)
	}
}

func (b *Bytes) sourceLimit() int64 { b.mu.Lock(); defer b.mu.Unlock(); return b.cfg.MaxSourceBytes }
func (b *Bytes) Configure(limit int64, sources int) {
	if limit <= 0 {
		limit = 512 << 20
	}
	if sources <= 0 {
	...
	b.mu.Lock()
	defer b.mu.Unlock()
	b.cfg.MaxSourceBytes, b.cfg.MaxSources = limit, sources
}
