/**
 * core.js — browser 指令集的平台无关核心（平台自有指令集，§5）
 *
 * 通过 adapter 注入宿主能力，同一套逻辑跑在：
 *   - Chrome 插件（chrome-adapter.js：chrome.tabs/windows/scripting/debugger/downloads）
 *   - desktop 壳（electron-adapter.js：BrowserWindow + webContents.debugger CDP）
 *
 * 指令集（13 action）：open/click/close/download/eval/get/network/read/screenshot/
 *   snapshot/tab/wait/sleep
 *   - eval/截图走 CDP（Runtime.evaluate / Page.captureScreenshot，DevTools 语义，
 *     不受页面 CSP 限制、可 await Promise）
 *   - 未实现能力（type/fill/press/滚动/导航等）→ unknown action，用 eval 注入 JS 替代
 ...
 *
 * adapter 契约（全部 async）：
 *   tabs:      active() / get(id) / list(windowId|undefined=当前窗口) /
 *              create({windowId, url, active}) / update(id, {url}|{active}) / remove(id)
 *              onUpdated.add(fn(tabId,{status})) / onUpdated.remove(fn)
 *              → tab 对象至少含 {id, windowId, title, url, active}
 *   windows:   listNormal() → [{id, incognito}] / get(id) → {id, incognito} /
 *              createIncognitoUnfocused() → {id}（失败抛带指引的 Error）
 *   evalIn:    (tabId, func, args) → 页面主世界执行 func(...args) 的返回值
 *   cdp:       attach(tabId) / send(tabId, method, params) / detach(tabId)
 *   downloads: onceCreated(fn(item)) → cancel() / searchComplete(filenameRegex) → [{filename}]
 *   store:     get(key) → obj|null / set(key, obj|null)   （工作区状态持久化）
 *   settings:  () → {incognito?: bool, background?: bool}（工作区模式开关）
 */
...
// 子命令 = argv[0]，子命令参数 = argv.slice(1)。

export function createBrowserHandler(adapter) {
  async function browserHandler(ctx, toolData) {
    ...
    if (!flagSet) {
      // 未实现与未知子命令统一报 unknown action；supported 列表只列真实支持集
      return err0(`browser: unknown action "${action}" (supported: ${ALL_ACTIONS.join(", ")}; use eval <js> for input/navigation/scroll workarounds)`);
    }
    ...
    try {
      return await executeAction(action, pa, ctx);
    } catch (e) {
  ...
  //   两者都关        → 当前激活标签页（协作模式）
  // 状态持久化 adapter.store（宿主休眠/重启后恢复；tab 被误关后自动重建）。
  // 工作区模式所有操作不激活 tab、不改变窗口焦点——绝不与用户抢操作。

  const WORKER_STORAGE_KEY = "aicWorker";

  async function loadWorkerState() {
    return (await adapter.store.get(WORKER_STORAGE_KEY)) || null;
  }

  async function saveWorkerState(state) {
    await adapter.store.set(WORKER_STORAGE_KEY, state || null);
  }

  // 当前是否 AI 工作区模式（后台 or 隐私）
  async function workerMode() {
    const s = await adapter.settings();
  ...
  async function getTargetTab() {
    ...
    return adapter.tabs.active();
  }

  // 找非无痕 normal 窗口（普通窗口模式避免落在用户的无痕窗口）
  async function findNormalWindow() {
    const wins = await adapter.windows.listNormal();
  ...
  async function ensureWorkerTab(incognito) {
    ...
    if (st && st.winIncognito === incognito) {
      try {
        const tab = await adapter.tabs.get(st.tabId);
        const win = await adapter.windows.get(tab.windowId);
    ...
    if (incognito) {
      ...
      if (existing) {
        ...
        const win = await adapter.windows.createIncognitoUnfocused();
        winId = win.id;
      }
    } else {
    ...
    let tab;
    if (incognito) {
      const tabs = await adapter.tabs.list(winId);
      tab = tabs[0] || (await adapter.tabs.create({ windowId: winId, active: false }));
    } else {
      tab = await adapter.tabs.create({ windowId: winId, url: "about:blank", active: false });
    }
  ...
  async function execInTab(tabId, func, args) {
    // 执行失败由 adapter reject 表达（错误信息透传），undefined 是合法返回值
    return adapter.evalIn(tabId, func, args || []);
  }
    ...
    return new Promise((resolve) => {
      const listener = (updatedTabId, info) => {
        if (updatedTabId === tabId && info.status === "complete") {
          ...
          adapter.tabs.onUpdated.remove(listener);
          setTimeout(resolve, 300);
        }
      };
      // 超时路径同样移除 listener：只清 timeout 会残留到该 tab 下次 complete
      const timeout = setTimeout(() => {
        adapter.tabs.onUpdated.remove(listener);
        resolve();
      }, ms);
      adapter.tabs.onUpdated.add(listener);
    });
  }
  ...
  async function actClose(pa) {
    ...
    const tabs = await adapter.tabs.list(wtab.windowId);
    if (tabs.length <= 1) {
      return { content: "Cannot close last AI tab", attrs: { action: "close", closed: "false" } };
    }
    await adapter.tabs.remove(wtab.id);
  ...
  async function actDownload(pa) {
    ...
    const downloadPromise = new Promise((resolve, reject) => {
      ...
      cancelWatch = adapter.downloads.onceCreated((item) => {
        clearTimeout(timer);
        resolve(item);
      });
    });
    ...
    if (clickResult?.stale || clickResult?.missing) {
      // click 失败路径：解除下载监听并吞掉悬挂的超时 reject——否则 30s 后
      // downloadPromise 成 unhandled rejection，且 electron 端 onceCreated 的
      // listener 不自移除，残留 createdListeners 之后每次下载触发死回调
      cancelWatch();
  ...
  async function cdpEval(tabId, js) {
    ...
    try {
      const res = await withTimeout(
        adapter.cdp.send(tabId, "Runtime.evaluate", {
          expression: js,
          awaitPromise: true,
          returnByValue: true,
        }),
  ...
  function extractReadableText(html) {
    // 宿主可能无 DOMParser（MV3 SW 实测 "DOMParser is not defined"）——正则剥离：
  ...
  async function actTab(pa) {
    ...
    if (sub === "new") {
      const tab = await adapter.tabs.create({ windowId: winId, active: false });
    ...
    if (sub === "close") {
      ...
      await adapter.tabs.remove(targetId);
      if (targetId === wtab.id) {
        // 当前工作 tab 被关才清空工作区状态：下一条指令重建干净的 AI tab（避免
        // 目标漂移到用户 tab）；关背景 tab 时保留 AI 正在看的页面上下文
        await saveWorkerState(null);
      }
  ...
  async function actTabCoop(sub, pa) {
    if (sub === "new") {
      await adapter.tabs.create({ active: true });
      const tabs = await adapter.tabs.list(undefined);
    ...
    if (sub === "close") {
      ...
      if (nArg !== undefined) {
        ...
        await adapter.tabs.remove(tabs[n - 1].id);
      } else {
        const active = await adapter.tabs.active();
        await adapter.tabs.remove(active.id);
      }
    ...
    if (!isNaN(n)) {
      ...
      await adapter.tabs.update(tabs[n - 1].id, { active: true });
  ...
  async function actWait(pa) {
    ...
    if (pa.flags["url"]) {
      ...
      return await waitCondition(`url ${glob}`, WAIT_DEFAULT_MS, async () => {
        const t = await adapter.tabs.get(tab.id);
  ...
  // waitPoll 轮询执行注入函数（修复史：注入 new Function 拼 JS 在页面 CSP 无
  // unsafe-eval 时抛 EvalError → 永远 false → wait <selector> 必超时。
  // 结构化参数直传（func 是真实函数体序列化，不受 CSP 动态执行限制）。
  ...
  async function waitDownload(filename, ms) {
    const start = Date.now();
    const re = filename.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    while (Date.now() - start < ms) {
      const items = await adapter.downloads.searchComplete(re);
