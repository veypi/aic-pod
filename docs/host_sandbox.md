...
- **deny 隔离**（2026-09-05）：deny 表（fsauth `defaultDenyPaths` + cfg `fs_deny_paths`）接入 exec 沙箱——沙箱默认读全开（seatbelt `allow default` / bwrap 整机 ro-bind），此前 fsauth deny 只约束 fs 工具（文件 API），exec 进程 `cat` 可直读 `.ssh` 等全部凭证（实测报告触发）。链：`Policy.DenyPatterns()`（预展开模式快照，与 fs 判定同一份名单单源）→ `StartOptions.DenyPaths` → 沙箱 profile：
