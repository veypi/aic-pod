...
fs_deny: []         # 路径 glob 拒绝名单（0 级不可读写，恒优先，默认表之上只加不减）
fs_allow: []        # 路径写白名单（2 级写区）
net_policy: open    # open（默认）| deny（锁定模式：沙箱出站 localhost-only）
net_deny: []        # host:port 出站拒绝名单（恒优先）
...
统一判定式：**deny 命中 → 拒；policy=open → 放；policy=deny → 仅 allow 放行**。条目形态：fs = 路径 glob（canonical 判定）；net/ssh = `host:port`（host 小写/尾点/IP 经 netip 归一；port 数字或 `*`；bare host 归一 `host:*`；`user@` 前缀剥除——用户是认证细节不是网络目标）。
