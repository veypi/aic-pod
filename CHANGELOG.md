...
- **read 数值参数回退语义（§4.2，行为变更）**：fs read 的 offset/limit 越界/非法不再返回 `state=error`——就近回退为有界窗口、说明写入 `attrs.note`（弱模型「越界-报错-纠偏」一轮往返的成本高于宽容回退）。规则：`offset>total` → 文件尾窗口（后 100 行，`readTailLines`）；`offset<1` → 文件头窗口（前 100 行，`truncated`+`hint` 可继续翻页）；`limit<1` → 按缺省 1000 执行；空文件（0 行）→ 空正文 + note（此前 offset=1 读空文件也报错）。note 文案例：`offset 195 exceeds 194 lines; returned lines 95-194`、`offset 0 must be >= 1; returned lines 1-100`、`limit 0 must be >= 1; used the default limit (1000)`。覆盖整读与 >8MB 流式双路径——流式尾窗口在总行数已知后二次扫描（免常驻尾环缓冲）；顺带把两处「512KB 预算」陈旧注释改正为实值 128KB。测试：一致性向量 `testdata/vectors/fs_read.json` 三例改期望 + `TestReadFallback` 新用例（头/尾窗口、limit 缺省、空文件、offset==total 边界）。page 端（aic 仓 `ui/assets/libs/page_fs.js`）同语义同步。
- **目录列举容忍不可 stat 的子项（修复 Windows 文件窗口打不开）**：`list` 逐条 `dir.Lstat` 失败即整体报错——Windows 上系统联结（`C:\Users\All Users`/`Default User`、用户配置目录里的 `Application Data`、`Local Settings` 等旧版兼容联结）与独占文件（`pagefile.sys`、`hiberfil.sys`、`NTUSER.DAT`、`ntuser.dat.LOG*`）可枚举但拒绝打开，于是 `C:\`、`C:\Users`、`C:\Users\<user>`、`C:\Windows`（`CSC`）等目录一列就 permission_denied/filesystem_error，前端文件窗口/Picker 直接打不开（vcore 视图不逐条 stat，平台/AI 路径正常）。现回退目录扫描自带元数据（Windows 由目录枚举缓存提供，无需再访问条目），连它都不可得才跳过该条；读侧遍历（find/search）对子项取信息失败一律跳过（此前只放过 permission_denied，`NTUSER.DAT` 这类 filesystem_error 会让从 `C:\` 起搜的 find 整单失败），写侧（copy/remove/move）保持严格。测试：`TestWindowsListToleratesUnstatableChildren`、`TestFindToleratesUnreadableChildren`。
- **Windows 隐藏属性与目录联结口径（Explorer 同款）**：`list`/`find` 缺省跳过 HIDDEN 属性条目（与点开头同口径，`hidden:true` 仍可显示）——此前 `My Music`、`NTUSER.DAT`、`pagefile.sys`、`System Volume Information` 等系统条目会出现在缺省列表；「指向目录的 reparse」（junction/目录符号链接）改按 directory 归类——此前 Go 对 surrogate 型 reparse 不设目录位（ModeIrregular），前端将其显示成伪文件（文件图标、无下钻箭头）。测试：`TestWindowsListSkipsHiddenAttributeEntries`、`TestWindowsListClassifiesDirectoryLinksAsDirectories`。
- **修复跨机时效校验的时钟容差（exec「unauthorized ... validity window」）**：设备端 `hosts_nats.Verify` 的窗口判定此前零余量——平台签发 `AuthorizationUntil=now+30min`，对时校准误差一旦落在负侧（毫秒级即可）就把整段会话的请求全部拒掉（实测 win 本机时钟比平台快 55.4s，靠对时补偿；重启/重对后 3ms 差异即从全拒变全通）；且 `Deadline=now+timeout`（平台 timeout 上限 10min）撞上设备 5min 上限，所有 timeout>5min 的调用（如 curl 600s）必然被拒（实测同一 curl：120s 通过、400s 立即 unauthorized）。现窗口判定加 `clockSlack=2min` 容差、Deadline 上限提到 15min（覆盖平台最长等待）。测试：`TestVerifyClockSlackToleratesCalibrationError`。（平台侧未动，设备端重建即生效。）
- **修复管道流 EOF 语义（curl/ssh/scp 短输出「执行永不完成」）**：`spawnBody.Read` 在流已 EOF 后把 `Wait()` 的 nil 当错误返回（`(0, nil)`）——`io.Copy` 遇到 (0, nil) 会无限空转，于是 curl 无 `-o` 路径（嗅探窗先把短体吃到 EOF，其后 io.Copy 再读）任务体永不返回：执行条目长期 `running`（`bg_list` 可见），回包被拖到请求 wait 到期（win/macOS 均复现：timeout≤120s 尚有余量、timeout≥5min 时平台先超时 →「device request failed (effects may have occurred): context deadline exceeded」，而执行其实已完成、日志完整）。现 EOF 后持续返回 `io.EOF`（退出错误仅首读上报）。测试：`TestSpawnBodyEOFAfterSniffTerminatesCopy`（修复前以 `Read after EOF returned (0, nil): io.Copy spins forever` 失败）。
- **沙箱 deny 实例化共享一趟遍历（修复 win 端每条命令多付 ~17s）**：`denyCoverAll` 此前对每条含 `**` 的 deny 模式各走一趟递归枚举——内置 ~11 条 `**` 模式（`**/.env`、`**/id_rsa*`、`**/.ssh/**` 等）都在 `$HOME` 上各走一趟（win 实测一趟家目录枚举 1.9-2.6s），于是每次沙箱 exec 多付 ~17s（实测同一命令：沙箱 16.92s / 免沙箱 1.29s）。现按 walk 根（`denyWalkRoot`）分组共享遍历：同一快照下目标集合与逐模式遍历一致（文件逐条产出；目录命中即整棵子树覆盖、不再下探，子孙目标仍由 `pruneCoverTargets` 按父目标收敛），每调用只付一趟枚举；win 与 linux（bwrap 同一 `denyCoverAll`）同步受益。目录访问预算（`denyWalkMaxDirs`）改按共享遍历计：超限仍整体不可实例化（fail-closed）；命中剪枝使访问量低于逐模式旧实现。测试：`TestDenyWalkAllSharedTraversal`。
- **沙箱 deny 提速：实例化缓存 + codex 式持久 ACE + 阶段计时打点（win 沙箱每条命令 5.2s → 预期 ~0.3s）**：共享遍历（上一条）后每调用仍付一趟 `$HOME` 递归枚举（win 实测 ~1.9s，占沙箱命令 5.2s 的最大单项）。新增 `denyCoverAllCached`（生产路径：windows `ensureDenyACEs`、linux `planConfined`）：同一 deny 模式集在 `denyCoverCacheTTL`（默认 30s）内复用实例化结果——窗口内新建的匹配对象在下次重算前不会被打 deny ACE / 覆盖挂载（语义取舍，2026-09-23 已确认）；不可实例化与超预算错误一并缓存（确定性判定，fail-closed 不变）；测试径仍走 `denyCoverAll` 原始路径（与文件系统实时对齐）。缓存后实测剩余开销全在 deny ACE 的施加/撤销：79 个目标逐个 Get/SetNamedSecurityInfo 约 16ms，施加与撤销各 ~1.3s。现改为 **codex windows-sandbox-rs 同款的持久 ACE 模型**（参考 `codex-rs/windows-sandbox-rs`，2026-09-23）：稳定 deny SID（能力 SID 确定性派生，跨调用/重启不变）+ ACE 一旦打上就保留（不再按进程撤销——codex 理由：后代可能比 launcher 活得久；残留 ACE 只对稳定 SID 生效，宿主机其他进程不受影响）+ 幂等 ensure（先读校验、有则跳过：codex `add_deny_write_ace` 同款）；到期对账靠状态文件 `{UserConfigDir}/aic/sandbox/deny_acl_state.json`（新目标补打、旧目标撤销、实例化重算时既有目标并行校验——对象删除重建会丢 ACE）。稳态调用零 ACL 读写（缓存命中 + 状态差集），最坏一次命令只付一趟遍历。同时给沙箱与 exec 各阶段加时间戳日志（`Manager.SetLogf` 注入，pod 侧接 host 的 `OnLog`，未注入则全链静默）：deny 实例化（缓存命中/遍历耗时）、deny 对账（desired/checked/added/stale/cached/took）、windows plan 全阶段（validate/deny/ws-grant/tmp/token/job/total）与 spawn 耗时。测试：`TestDenyCoverAllCached`（TTL 内命中旧快照、过期重算、模式集变化即重算）、`TestWindowsDenyWriteACEAndCleanup`（ACE 持久保留 + 幂等不重复）、`TestWindowsDenyStaleRevocation`（目标集变化时旧目标对账撤销）。
