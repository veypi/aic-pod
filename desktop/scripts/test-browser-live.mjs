...
const server=http.createServer((req,res)=>{if(req.url==='/file'){res.writeHead(200,{'Content-Type':'text/plain','Content-Disposition':'attachment; filename="hello.txt"'});res.end('download fixture');}else{res.writeHead(200,{'Content-Type':'text/html'});res.end(fixture)}});
await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));
let win,provider;
try{
 await app.whenReady();win=new BaseWindow({show:false,width:900,height:700});const platformView=new WebContentsView();win.contentView.addChildView(platformView);
 provider=await startBrowserServer({host:{win,platformView}});const {adapter}=provider;
