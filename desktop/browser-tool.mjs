...
import { serveBrowserLive } from './browser/live-socket.mjs';
import crypto from 'node:crypto';
import path from 'node:path';
import os from 'node:os';
import { createDirectBrowser } from './browser/direct.mjs';
...
export async function startBrowserServer({ host, log = () => {} } = {}) {
  const adapter = createElectronAdapter(host),
    handler = createBrowserHandler(adapter);
  const direct = createDirectBrowser(adapter);
  ...
  const server = net.createServer((conn) => {
    ...
    conn.on('data', (chunk) => {
      ...
      let req;
      try {
        req = JSON.parse(buffer.slice(0, lineEnd));
      } catch {
        reply({ state: 'error', error: 'invalid json' });
        return;
      }
      if (req.token !== token) {
        reply({ state: 'rejected', error: 'invalid token' });
        return;
      }
      if (req.direct === true && req.live === true) {
        serveBrowserLive(conn, direct, req).catch(() => conn.destroy());
        return;
      }
      if (req.direct === true) {
        direct
          ...
          .catch((e) =>
            reply({
              error: {
                ...
                message: e.message,
                retry: 'never',
                effect: e.effect || (e.code ? 'none' : 'unknown'),
              },
            }),
          );
        return;
      ...
      if (!sessionDir) {
        reply({ state: 'rejected', error: 'invalid session_dir' });
        return;
      }
      const startedAt = Date.now();
      Promise.resolve()
        .then(async () => {
          if (req.control === 'session_end') {
            handler.endSession(req.session_id);
            reply({ state: 'completed', content: 'session released' });
            return;
          }
          const res = await handler(
            {
              ...
              startedAt,
              authorizedFile: req.authorized_file,
        ...
        .catch((e) => {
          ...
          reply(
            envelope(
              errorResult(
                { domain: 'browser', op: req.argv?.[0] || 'unknown' },
                e,
                'unknown',
              ),
            ),
  ...
  return { port, token, server, adapter, direct };
}
