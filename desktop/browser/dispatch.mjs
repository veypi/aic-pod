...
export function createBrowserDispatcher(adapter, handle, bounded) {
  ...
  async function failure(job, code, message, detail, performed = false) {
    ...
    if (code === 'dialog_open')
      r.error.recovery =
        'Use dialog accept or dialog dismiss on this target; do not replay the interrupted operation.';
    ...
    for (const job of queue.splice(0))
      reply(
        ...
        failure(
          job,
          'browser_busy',
          'A browser operation is suspended; resolve its dialog or close its target before issuing more input.',
        ),
      );
  }
  function suspend(job, dialog) {
    ...
    reply(
      ...
      failure(
        job,
        dialog ? 'dialog_open' : 'timeout',
        dialog
          ? 'Page execution is paused by a JavaScript dialog.'
          : 'The operation exceeded its deadline; its outcome may still be pending.',
        dialog,
        job.ctx.actionStarted ? 'unknown' : false,
      ),
    );
  ...
  const unsubscribe = adapter.onDialog?.(({ tab, dialog }) => {
    if (!dialog) return;
    for (const job of [active, ...controls])
      if (job?.ctx.nativeTarget === tab) suspend(job, dialog);
  });
  function start(job, lane) {
    const predecessors =
      lane === 'control' ? [active, ...controls].filter(Boolean) : [];
    if (lane === 'normal') active = job;
    if (lane === 'control') controls.add(job);
    job.ctx.waitForResume = async (tab) => {
      const waiting = predecessors.filter(
        (previous) => previous.ctx.nativeTarget === tab,
      );
      ...
      const original = waiting[0],
        r = original.ctx.uiResult;
      if (r)
        return {
          request_id: original.ctx.msgID,
          op: r.op,
          state: r.state,
          action: r.action,
          error: r.error,
    ...
    const end = Math.min(
      job.ctx.deadline ? Date.parse(job.ctx.deadline) : Infinity,
      (job.ctx.startedAt || Date.now()) + job.op.options.timeout_ms,
    );
    job.timer = setTimeout(() => suspend(job), Math.max(0, end - Date.now()));
    job.done = Promise.resolve()
      .then(() => handle(job.ctx, job.request))
      .catch((e) =>
        failure(
          job,
  ...
  function drain() {
    if (!active && !controls.size && queue.length)
      start(queue.shift(), 'normal');
  }
  function dispatch(ctx, request) {
    let op;
    try {
      op = parse('browser', request.argv);
    } catch {
    ...
    return new Promise((resolve) => {
      const job = {
        ctx: { ...ctx, startedAt: ctx.startedAt || Date.now() },
        request,
        op,
        resolve,
        responded: false,
      ...
      const suspended =
        active?.suspended || [...controls].some((c) => c.suspended);
      if (control.has(op.op) && suspended) {
        if ([...controls].some((c) => !c.responded)) {
          reply(
            job,
            failure(
              job,
              'browser_busy',
              'A dialog control is already in progress.',
            ),
          );
          return;
        }
